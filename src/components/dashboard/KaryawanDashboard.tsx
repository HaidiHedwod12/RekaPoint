  ...
  return (
    <div className="min-h-screen py-8 relative">
      ...
      <div className="fixed top-20 right-10 opacity-10">
        <MapPinIcon className="w-72 h-72 text-cyan-400" />
      </div>
      <div className="fixed bottom-10 left-10 opacity-10">
        <DocumentTextIcon className="w-56 h-56 text-blue-400" />
      </div>
      <div className="fixed top-1/3 left-1/4 opacity-5">
        <GlobeAltIcon className="w-80 h-80 text-purple-300" />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <motion.div
            ...
            className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-green-500 to-teal-600 rounded-full mb-6 pulse-glow"
          >
          ...
          >
            <div className="inline-flex items-center space-x-2 glass-effect px-6 py-3 rounded-full border border-cyan-500/30">
              <MapPinIcon className="w-5 h-5 text-cyan-400" />
              <span className="text-sm text-gray-400">Jabatan: </span>
              <span className="text-sm text-cyan-300 font-medium">{user?.jabatan}</span>
            </div>
            ...
            <div className="flex justify-center space-x-4 mb-4">
              <select
                ...
                className="px-3 py-2 glass-effect border border-gray-600/50 rounded-lg text-white bg-slate-800/50 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400 [&>option]:bg-slate-800 [&>option]:text-white"
              >
              ...
              {stats.canViewPoin ? (
                <>
                  <div className={`glass-effect px-4 py-2 rounded-lg border transition-all duration-300 ${poinStatus.bgColor}`}>
                    <div className={`text-lg font-bold ${poinStatus.color}`}>
                      {stats.totalPoin}/{stats.minimalPoin}
                    </div>
            ...
            {!stats.canViewPoin && (
              <div className="mt-4 glass-effect px-4 py-2 rounded-lg border border-yellow-500/20 bg-yellow-500/5">
        ...
        >
          ...
          <div className="glass-effect border border-cyan-500/20 rounded-xl p-6 hover:border-cyan-400/50 transition-all duration-300">
            <div className="text-center text-gray-400">
              <CalendarIcon className="w-12 h-12 mx-auto mb-4 text-cyan-400/50" />
              {stats.totalActivities === 0 ? (
                <>
                  <p>Belum ada aktivitas yang dicatat untuk periode ini</p>
                  <p className="text-sm mt-2">Klik "Tambah Aktivitas" untuk mulai mencatat</p>
                </>
              ) : (
                <>
                  <p>Total {stats.totalActivities} aktivitas tercatat</p>
                  {stats.canViewPoin && (
                    <p className="text-sm mt-2">Poin: {stats.totalPoin}/{stats.minimalPoin}</p>
                  )}
                </>
              )}
            </div>
