...
export const Kalender: React.FC = () => {
  ...
  const getActivitiesForDate = (date: Date) => {
    ...
    return aktivitas.filter(activity => {
      // activity.tanggal is already YYYY-MM-DD
      return activity.tanggal.startsWith(dateStr);
    });
  };
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-col items-center justify-center h-full">
                          <span className={`text-sm font-medium ${isSelected ? 'text-cyan-300' : 'text-gray-300'
                            }`}>
                            {day.getDate()}
                          </span>
                          {dayActivities.length > 0 && (
